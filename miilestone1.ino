...
void loop() {
  ...
  if(ps2x.Button(PSB_PAD_UP)) {
    openGripper(); // Open the gripper when PSB_R1 is pressed
  } else {
    closeGripper(); // Close the gripper when PSB_R1 is not pressed
  }
...
void openGripper() {
    for(int pos = 40; pos < 140; pos += 1) { // Open gripper
      myservo.write(pos);
      delay(40);
    }
}
...
void closeGripper() {
  for(int pos = 140; pos >= 40; pos -= 1) { // Close gripper
    myservo.write(pos);
    delay(40);
  }
}